...
// Función para insertar datos en la tabla 'Biblioteca'
async function insertData() {
  try {
    // Datos para insertar en la tabla 'Biblioteca'
    const dataToInsert = [
      {idUsuario:'1',Estado:'AAA',Fecha:'2024/04/18'}
    ];

    // Insertar datos en la tabla 'Biblioteca'
    const { data, error } = await supabase.from('Biblioteca').insert(dataToInsert).select();
...
app.get('/data', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('JBiblioteca').select('*');
