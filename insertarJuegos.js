...
async function insertData() {
  try {
    // Datos para insertar en la tabla 'Juegos'
    const dataToInsert = [
      { Nombre: 'silent hill', Genero: 'survival-horror',Descripcion:'AAABBB', Precio:'900.00',Lanzamiento:'2024/04/18',Desarrollador:'AAABBB',Clasificacion:'C' }
    ];

    // Insertar datos en la tabla 'Juegos'
    const { data, error } = await supabase.from('Juegos').insert(dataToInsert).select();
