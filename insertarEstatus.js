...
// Función para insertar datos en la tabla 'estatus'
async function insertData() {
  try {
    // Datos para insertar en la tabla 'Estatus'
    const dataToInsert = [
      {tipoEstatus:'Pagado'}
    ];

    // Insertar datos en la tabla 'Estatus'
    const { data, error } = await supabase.from('Estatus').insert(dataToInsert).select();
